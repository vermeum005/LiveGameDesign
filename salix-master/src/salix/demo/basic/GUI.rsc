...
Model init() = <"Options", "", grammar2mode("PuzzleScript", #PSProgram), "", [], [], [], "">;
...
{
	model.mtype = "Selection";
	return model;
}
Model update(AnalyseGame(), Model model)
{
	n = PS_implode(readGame());
	g = FM_implode(FM_parse());
	for (Feature r <- g.features)
	{
